...
public class MovieDatabase {
	...
	public void insertData(Movie mov) {
		...
		newMovie.put("title", mov.getTitle());
		...
		if (cursor.getCount() > 0) {
			db.update("movies", newMovie,
					"id" + "=?", new String[] { mov.getId()
							+ "" });
		} else {
	...
	public void removeMovie(Movie mov) {
		String whereClause = "id" + " = '" + mov.getId() + "' AND "
				+ "title" + " = '" + mov.getTitle() + "' AND "
	...
	public ArrayList<Movie> getAllItems() {
		...
		Cursor cursor = db.query("Movies", new String[] {
				"id", "title", "rating"}, null, null, null, null, null);
